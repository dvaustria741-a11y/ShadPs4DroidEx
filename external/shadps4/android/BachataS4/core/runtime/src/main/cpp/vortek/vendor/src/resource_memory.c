        ...
        if (hasExternalMemoryFd) {
            ...
            if (hasExternalMemoryDMABuf) {
                ...
                if (fd > 0) {
                    int dupFd = dup(fd);
#ifdef BACHATA_VORTEK_SERVER
                    /* Track original + transferred-to-Vulkan dup. */
#endif
...
#ifdef BACHATA_VORTEK_SERVER
                        backingType = "external";
                        /* onAlloc runs later; record FD pair after onAlloc below. */
#endif
                    }
                    else {
                        CLOSEFD(fd);
...
#ifdef BACHATA_VORTEK_SERVER
    VortekGpuTrack_onAlloc(resourceMemory, memoryInfo->memoryTypeIndex, backingType);
    if (resourceMemory->fd > 0) {
        VortekGpuTrack_onExternalFdEvent(resourceMemory, resourceMemory->fd, resourceMemory->fd,
                                         "import");
    }
#endif
    return resourceMemory;
...
void ResourceMemory_free(VkContext* context, VkDevice device, ResourceMemory* resourceMemory) {
...
#ifdef BACHATA_VORTEK_SERVER
    /* false => destroy deferred until GPU completion (Mode B) or FHD pin. */
    if (!VortekGpuTrack_onFree(resourceMemory, device)) {
        return;
    }
#endif
...
#ifdef BACHATA_VORTEK_SERVER
        VortekGpuTrack_noteDirectDestroyBufferBypass((void*)resourceMemory->buffer, (void*)device,
                                                     "ResourceMemory_free");
        if (VortekGpuTrack_onDestroyBuffer((void*)resourceMemory->buffer, (void*)device)) {
    ...
    if (resourceMemory->fd > 0) {
        VortekGpuTrack_onExternalFdEvent(resourceMemory, resourceMemory->fd, resourceMemory->fd,
                                         "close");
        /* pin may refuse close; still skip CLOSEFD if pin blocked free above. */
    }
#endif
