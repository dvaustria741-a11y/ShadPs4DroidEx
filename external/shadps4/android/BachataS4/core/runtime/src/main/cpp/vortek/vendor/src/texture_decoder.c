...
#ifdef BACHATA_VORTEK_SERVER
        if (VortekGpuTrack_onDestroyImage((void*)targetImage->image, (void*)device)) {
            vulkanWrapper.vkDestroyImage(device, targetImage->image, NULL);
        }
#else
        vulkanWrapper.vkDestroyImage(device, targetImage->image, NULL);
#endif
    }
    if (targetImage->buffer) {
#ifdef BACHATA_VORTEK_SERVER
        VortekGpuTrack_noteDirectDestroyBufferBypass((void*)targetImage->buffer, (void*)device,
                                                     "TextureDecoder");
        if (VortekGpuTrack_onDestroyBuffer((void*)targetImage->buffer, (void*)device)) {
            vulkanWrapper.vkDestroyBuffer(device, targetImage->buffer, NULL);
        }
#else
        vulkanWrapper.vkDestroyBuffer(device, targetImage->buffer, NULL);
#endif
...
#ifdef BACHATA_VORTEK_SERVER
        /* Not ResourceMemory-backed; free immediately but log for freeflight audit. */
        VortekGpuTrack_noteDirectDestroyBufferBypass((void*)targetImage->memory, (void*)device,
                                                     "TextureDecoder_freeMemory");
#endif
        vulkanWrapper.vkFreeMemory(device, targetImage->memory, NULL);
    }
    MEMFREE(targetImage);
}
