...
VkPhysicalDevice bachataHostPhysicalDevice = VK_NULL_HANDLE;
...
#ifdef BACHATA_VORTEK_SERVER
static VKAPI_ATTR VkBool32 VKAPI_CALL bachataDebugUtilsCallback(
    ...
    if (messageTypes & VK_DEBUG_UTILS_MESSAGE_TYPE_DEVICE_ADDRESS_BINDING_BIT_EXT) {
        ...
        const VkDeviceAddressBindingCallbackDataEXT* bind = NULL;
        const void* pNext = pCallbackData->pNext;
        while (pNext) {
            const VkBaseInStructure* base = (const VkBaseInStructure*)pNext;
            if (base->sType == VK_STRUCTURE_TYPE_DEVICE_ADDRESS_BINDING_CALLBACK_DATA_EXT) {
                bind = (const VkDeviceAddressBindingCallbackDataEXT*)pNext;
                break;
            }
            pNext = base->pNext;
        }
        if (bind) {
            VortekGpuTrack_noteAddressBinding((uint64_t)bind->baseAddress, (uint64_t)bind->size,
                                              (int)bind->bindingType, (uint32_t)bind->flags);
        } else if (pCallbackData->pMessage) {
            println("GPU_ADDRESS_BIND_MSG %s", pCallbackData->pMessage);
        }
        return VK_FALSE;
    }
    if (pCallbackData->pMessage &&
        ...
        println("VulkanUtils: %s", pCallbackData->pMessage);
    }
    return VK_FALSE;
}
#endif
...
void initVulkanInstance(VkContext* context, VkInstance instance, const VkApplicationInfo* applicationInfo) {
    ...
    /* Messenger only when address-binding report dig is ON (default OFF). */
    if (VortekGpuTrack_gpuAddressBindingReportEnabled() &&
        vulkanWrapper.vkCreateDebugUtilsMessengerEXT) {
        VkDebugUtilsMessengerCreateInfoEXT utilsInfo = {0};
        utilsInfo.sType = VK_STRUCTURE_TYPE_DEBUG_UTILS_MESSENGER_CREATE_INFO_EXT;
        utilsInfo.messageSeverity = VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT |
                                    VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT |
                                    VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT;
        utilsInfo.messageType = VK_DEBUG_UTILS_MESSAGE_TYPE_DEVICE_ADDRESS_BINDING_BIT_EXT;
        utilsInfo.pfnUserCallback = bachataDebugUtilsCallback;
        VkResult utilsResult = vulkanWrapper.vkCreateDebugUtilsMessengerEXT(
            instance, &utilsInfo, NULL, &context->debugUtilsMessenger);
        if (utilsResult != VK_SUCCESS) {
            context->debugUtilsMessenger = VK_NULL_HANDLE;
            println("debug_utils messenger create failed: %d", (int)utilsResult);
        } else {
            println("debug_utils messenger enabled (device_address_binding)");
        }
    } else {
        println("GPU_ADDRESS_BINDING_REPORT enabled=0 (no debug_utils messenger)");
    }
#endif
}
...
void initVulkanDevice(VkContext* context, VkPhysicalDevice physicalDevice, VkDevice device) {
    ...
    bachataHostPhysicalDevice = physicalDevice;
...
#ifdef BACHATA_VORTEK_SERVER
            VortekGpuTrack_noteDirectDestroyBufferBypass(handle, (void*)device, "destroyVkObject");
            if (VortekGpuTrack_onDestroyBuffer(handle, (void*)device)) {
