...
if [ "$3" = x64 ]; then
    ...
    exec "$loader" --library-path "$root/host" "$4" /data/user/0/Ali.kytyps4/files/vulkan-map-probe-x64
fi
exec "$loader" --library-path "$root/host" /data/user/0/Ali.kytyps4/files/vulkan-map-probe
